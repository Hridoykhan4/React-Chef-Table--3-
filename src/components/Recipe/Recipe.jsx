    ...
    <div class="card card-compact bg-base-100 shadow-xl p-1">
      ...
      <div class="card-body">
        ...
        <ul className="list-inside list-disc">
            {
                ingredients.map((ingridient, idx) => <li key={idx} className="text-gray-800">{ingridient}</li>)
            }
        </ul>
        ...
        <div className="flex items-center gap-10">
            <div className="flex items-center gap-1">
                <span className="text-xl text-black font-semibold"><CiClock2 /></span>
                <span className="font-semibold">{preparing_time} minutes</span>
            </div>

            <div className="flex items-center gap-1">   
                <span><MdOutlineEnergySavingsLeaf /></span>
                <span className="font-semibold">{calories} calories</span>
            </div>
        </div>


        <button id={`btn-${recipe_id}`} onClick={() => handleCook(recipe)} className="btn color-bg btn-success self-start my-3 text-white">Want to Cook</button>
